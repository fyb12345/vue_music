<template>
  <div class="music-list">
    ...
    <h1 ref="singerName" class="title">
      {{ title }}
    </h1>
    <div ref="bgImage" :style="bgStyle" class="bg-image">
      <div v-show="songList.length>0" class="play-wrapper">
    ...
    <div ref="layer" class="bg-layer"></div> <!-- 新建一个蒙版元素, 加上ref值 -->
    <Scroll ref="list" :data="songList" :listenScroll="listenScroll" :probeType="probeType" class="list"
            @scroll="scroll">
      <!--  scroll事件就会冒泡上来 我们再监听一个scroll事件  传入自定义的数据, 激活scroll组件的功能-->
      <div>
        <!-- 传入数据, 使得scroll组件知道内容的量    设置该组件的top值-->
        <div class="song-list-wrapper">
          <songlist v-bind:singername="title" v-bind:songs="songList" @select="selectItem"></songlist>
        </div>
      </div>
