<template>
  <div v-show="playlist.length>0" class="player">
    ...
    > <!--        配置几个钩子,方便实现更复杂的动画效果,这些钩子就是模拟组件加载的几个过程            -->
      <div v-show="fullScreen" class="normal-player"><!-- 全屏播放器 -->
        <div class="background">
          <!-- 大的背景图 占据所有 数据就用songData里面的songPic-->
          <img alt="" height="100%" v-bind:src="songData.songPic" width="100%">
        </div>
        <div class="top">
        ...
        >
          ...
          <div ref="middleL" class="middle-l">
            <div ref="cdWrapper" class="cd-wrapper"> <!-- 歌曲唱片设计 -->
              <div :class="cdClass" class="cd">
            ...
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p v-for="(line,index) in currentLyric.lines"
                   ref="lyricLine"
                   v-bind:key="index"
                   :class="{'current': currentLineNum ===index}" class="text">{{ line.txt }}</p>
              </div>
            </div>
          </scroll>
        </div>
        <div class="bottom">
          <div class="progress-wrapper">
            ...
            <span class="time time-r">{{ transformPlayTime(songData.playTime) }}</span>
          </div>
    ...
    <transition name="mini">
      <div v-show="!fullScreen" class="mini-player" @click="open"><!-- 迷你播放器 -->
        <div class="icon">
          <img :class="cdClass" alt="" height="40" v-bind:src="songData.songPic" width="40">
          <!-- 小型唱片 也添加一个cdClass-->
        </div>
        <div class="text">
    ...
    <audio ref="audio" v-bind:src="songData.m4aUrl" @canplay="ready" @ended="end"
  ...
  watch: {
    ...
    songData() {
      ...
      axios.post(`${this.basePath}api/getLyric`, JSON.stringify(data)).then((data) => {
        ...
        this.totalTime = Number(this.songData.playTime.split(":")[0]) * 60 + Number(this.songData.playTime.split(":")[1]);   if (this.playing) {//当歌曲处于播放状态的时候, 我们就播放歌词
